...
| **Level** | L3 — Maria (~25% of known audience) |
| **Phase** | 1 — Launch |
| **Purpose** | Immediate validation + mechanism teaser. Convert resignation into calm curiosity. |
| **Emotional Arc** | Mirror exhaustion → Validate dignity → Offer mechanism as neurological explanation → Quiet curiosity to continue |
| **Send Timing** | Day 0 (immediately after pop-up signup) |
| **Cadence Variant** | SLOWEST — E1→E2 gap 2 days (not 1). Zero urgency in any email. |
| **Tag** | `avatar_level3` |
| **Source Justification** | S2 Copy Strategy §Welcome E1 L3; GIULIANO-DEMANDS.md §3, §5; MASTER-DIAGNOSIS-COMPENDIUM.md §1.3 |
...
| Claim / Line | Source | Verbatim Quote Used |
|-------------|--------|-------------------|
| "He's never shown a sign of feeling wet or dirty in all these years" | MASTER-INGESTION-COMPENDIUM.md §2.4 Cat 1 #1 | L3 parent verbatim: "He's never shown a single sign of feeling wet or dirty." |
| "He's been in pull-ups for 9 years and never reacted to a single wet diaper" | MASTER-INGESTION-COMPENDIUM.md §2.4 Cat 1 #2 | L3 parent verbatim |
| "I live each day between hopelessness, and holding on to any drop of hope" | MASTER-INGESTION-COMPENDIUM.md §2.4 Cat 5 #6 | L3 parent verbatim |
| "He doesn't have full bladder awareness yet" | MASTER-INGESTION-COMPENDIUM.md §2.1 | Universal parent verbatim |
| "I wish I had read something like this years ago" | MASTER-INGESTION-COMPENDIUM.md §2.4 Cat 9 #8 | L3 parent verbatim (adapted) |
| "He's doing the best his body allows" | MASTER-INGESTION-COMPENDIUM.md §2.4 — Dignity-first rules | Adapted from dignity-first language guidelines |
...
| Welcome Flow E1 structure: Hook→Validation→Mechanism→CTA | GIULIANO-DEMANDS.md §3 | Section Map for E1-E2 |
| Lena sign-off SHORT for E1-E2 | GIULIANO-DEMANDS.md §3 | "Talk soon," closing |
| Forbidden: cheerleading, "your child WILL train", grief as sales | GIULIANO-DEMANDS.md §5, §8.5, §25 | Phase 4 / L3 forbidden language |
...
| **CTA Section** | Single button, left-aligned (not centered). 56px height. Full-width on mobile, 240px on desktop. The button sits alone with generous space above and below. No secondary CTA in E1 for L3 — one action, zero clutter. |
| **Footer** | Standard footer at 11px, 50% opacity. |
...
| Role | Hex | Name | Usage | Emotional Arc |
|------|-----|------|-------|---------------|
| **Background** | #FAF8F5 | Warm Ivory | Full email body background | Calm, uncluttered — a neutral space that doesn't demand attention |
| **Body Text** | #3A3530 | Deep Clay | Primary body copy | Soft black. Not #000 — too harsh. Clay adds warmth without warmth becoming "fake-cheerful." |
| **Headline** | #2B2724 | Dark Truffle | Subject line, section headers | One step darker than body text. Subtle hierarchy without hard contrast. |
| **Accent — Signal** | #C4A88B | Warm Sand | Signal Dot, vertical line indicator | The only saturated element. It appears sparingly — 4-6 times across the entire email. Desaturated enough to not feel "marketing." |
| **Secondary Accent** | #A8B5A0 | Sage Moss | Product layer lines, statistic separators | Cool complement to Warm Sand. Represents the mechanism — calm, scientific, unemotional. |
| **Muted Support** | #D4C5B2 | Pale Stone | Watercolor circle, faded backgrounds | Only in the background layer. The least visible but most essential — provides depth without structure. |
| **CTA Background** | #4A4540 | Warm Charcoal | CTA button fill | Dark enough to contrast with the background. Not #222 (too aggressive for L3). |
| **CTA Text** | #FAF8F5 | Warm Ivory | CTA button copy | Matches background for a calm, integrated button — not a "click me!" shout. |
| **Footer** | #8A8580 | Warm Grey | Footer text | Lowest optical weight — information, not persuasion. |
...
| Type | Font | Size | Weight | Tracking | Use |
|------|------|------|--------|----------|-----|
| **Display / Hook** | Lausanne (or premium sans-serif equivalent) | 24px / 36px on desktop | 300 (Light) | 0.02em | The opening hook — set at display size but in a very light weight to read as a whisper, not a headline |
| **Body** | Lausanne Text (or premium equivalent) | 17px / 20px on desktop | 400 (Regular) | 0.01em | All body copy. Generous leading: 1.6x. Keeps the reader moving slowly through the text. |
| **Section Headers** | Lausanne | 13px | 600 (Semibold) | 0.08em (tracked out) | Section labels (Hook, Validation, Mechanism) — set small, all lowercase, wide tracking to read as architectural notes, not headlines |
| **Statistics** | Lausanne Display | 48px | 200 (ExtraLight) | -0.02em | The big numbers in social proof — set extremely light to feel weightless, like data that carries no emotional load |
| **CTA Button** | Lausanne | 16px | 500 (Medium) | 0.04em | Button text — medium weight, slightly tracked out for readability at small size |
| **Footer** | Lausanne Text | 11px | 400 (Regular) | 0.02em | Standard footer sizing |
...
| Element | Visual Treatment | Placement |
|---------|-----------------|-----------|
| **Section Separator** | A single thin horizontal rule — 0.5px, #D4C5B2 at 50% opacity, 30% of content width, left-aligned. Not a full-width line. | Between every section. Each one sits 24px above and 24px below. |
| **Mechanism Indicator** | A 2px vertical line (#B8A99A, 60% opacity) inset 8px from left edge of the mechanism text block, spanning the full height of the block. | Left edge of the Mechanism section text block. |
| **Statistical Separator** | A thin horizontal rule — 0.5px, #A8B5A0 at 40% opacity, 50% of content width, left-aligned. | Between each stacked statistic block. |
| **Signal Dot Container** | Each of the three Signal Dots sits inside an invisible square frame (24px × 24px). The dot is vertically and horizontally centered within it. | Masthead (upper right), Mechanism section (left of key paragraph), Lower right (near CTA). |
| **Background Wash** | A single soft geometric shape — a circle (600px diameter) rendered at 6% opacity in #D4C5B2 — positioned behind the text, bleeding off the left edge. | Behind the Validation and Hook sections. Gives depth without drawing attention. |
...
|---------|-----------|---------|----------|--------|
| **Signal Dot #1** | Fade in from 0% to 15% opacity | On email open (static) | Instant | N/A |
| **Signal Dot #2** | Pulse once — scale from 100% to 110% and back | When the mechanism paragraph about 30-60 second feedback enters viewport | 1.2s | ease-out-expo |
| **Signal Dot #3** | Gentle breathing animation — scale oscillates between 100% and 105% | On section entry | Continuous, 4s cycle | ease-in-out-sine |
...
| Element | L3 Calibration | How It Differs from L1/L2 |
|---------|---------------|--------------------------|
| **Overall Tone** | "Quiet Dawn" — low contrast, desaturated, warm neutrals | L1 would use higher contrast with Pre-K appeal visual cues. L2 would use more structured layout with scientific diagram style. L3 uses the least saturation and the most negative space. |
| **Typography Weight** | Hook at 300 (Light), body at 400, stats at 200 (ExtraLight) | L1 would use 400/500 weights for urgency-driven feel. L2 would use variable weights for mechanism depth. L3 uses the lightest possible weights — nothing feels heavy or urgent. |
...
| Exponential easing curves | consolidated-design-skills-trimmed.md — "impeccable" §Motion | No bounce, no elastic — these would read as playful or promotional for L3. |
| Lena sign-off at body weight (14px, 400) | MASTER-DIAGNOSIS-COMPENDIUM.md §3.1 | Lena is a peer, not a brand authority. Typographically treating her sign-off at body weight reinforces this. |
